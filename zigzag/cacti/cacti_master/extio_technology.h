...
public:
  ...
  IOTechParam(InputParameter *, Mem_IO_type io_type, int num_mem_dq,
              int mem_data_width, int num_dq, int connection, int num_loads,
              double freq);
  ~IOTechParam();
  double num_mem_ca; /* Number of loads on the address bus
          based on total number of memories in the channel.For
          registered or buffered configurations, the num_mem_dq and num_mem_ca
          is per buffer. */
  ...
  double i_bias; /* Bias current (mA) (includes bias current for the whole
          memory bus due to RX Vref based receivers */
  ...
  double
      t_error_soc; // Timing error due to edge placement uncertainty of the DLL
  double t_skew_setup; // Setup skew between DQ/DQS or CA/CLK after deskewing
                       // the lines
  double t_skew_hold;  // Hold skew between DQ/DQS or CA/CLK after deskewing the
                       // lines
  double t_dqsq;       // DQ-DQS skew at the DRAM output during Read
  // double t_qhs;   //DQ-DQS hold factor at the DRAM output during Read FIXME:
  // I am commenting it as the variable is never used.
