...
public:
  ...
  bool capacity_wise; // true means the load on each channel is proportional to
                      // its capacity.
...
public:
  ...
  bool valid;
  // Functions
  channel_conf(MemCadParameters *memcad_params, const vector<int> &dimm_cap,
               int bandwidth, Mem_DIMM type, bool low_power);
