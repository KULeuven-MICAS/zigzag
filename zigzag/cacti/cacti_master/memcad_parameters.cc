...
MemCadParameters::MemCadParameters(InputParameter *g_ip) {
  ...
  capacity_wise = true; // true means the load on each channel is proportional
                        // to its capacity.
  first_metric = Cost;
  ...
  rd_2_wr_ratio = 2;
  same_bw_in_bob =
      true; // true if all the channels in the bob have the same bandwidth
  mirror_in_bob =
      true; // true if all the channels in the bob have the same configs
...
double MemoryParameters::IDD4R[2][4] = {{120, 135, 155, 175},
                                        {140, 140, 150, 160}};

double MemoryParameters::IDD4W[2][4] = {{100, 125, 145, 165},
                                        {156, 156, 176, 196}};

double MemoryParameters::IDD5[2][4] = {{150, 205, 210, 220},
                                       {190, 190, 190, 192}};

double MemoryParameters::io_energy_read
    [2][3][3][4] = // [ddr3:ddr4][udimm:rdimm:lrdimm][load
                   // 1:2:3][frequency 0:1:2:3]
    {{             // ddr3
      {            // udimm
...
int MemoryParameters::bandwidth_load[2][4] = {{400, 533, 667, 800},
                                              {800, 933, 1066, 1200}};

double MemoryParameters::cost[2][3][5] = {{{40.38, 76.13, INF, INF, INF},
...
double calculate_power(double load, double row_buffer_hr, double rd_wr_ratio,
                       int chips_per_rank, int frequency_index, int lp) {
  return 0;
}
...
channel_conf::channel_conf(MemCadParameters *memcad_params,
                           const vector<int> &dimm_cap, int bandwidth,
                           Mem_DIMM type, bool low_power)
    : memcad_params(memcad_params), type(type), low_power(low_power),
      bandwidth(bandwidth), latency(0), valid(true) {
  ...
  // bandwidth =
  // MemoryParameters::bandwidth_load[memcad_params->io_type][4-num_dimm_per_channel];
  // updating channel cost
  cost = 0;
  for (int i = 0; i < 5; ++i)
    cost += histogram_capacity[i] *
            MemoryParameters::cost[memcad_params->io_type][type][i];
...
void channel_conf::calc_power() {
  ...
  double read_ratio =
      memcad_params->rd_2_wr_ratio / (1.0 + memcad_params->rd_2_wr_ratio);
  ...
  energy_per_read =
      MemoryParameters::io_energy_read[current_io_type][type]
                                      [num_dimm_per_channel - 1]
                                      [bw_index(current_io_type, bandwidth)];
  energy_per_read /= (bandwidth / T_BURST);

  energy_per_write =
      MemoryParameters::io_energy_write[current_io_type][type]
                                       [num_dimm_per_channel - 1]
                                       [bw_index(current_io_type, bandwidth)];
  ...
  energy_per_access =
      read_ratio * energy_per_read + write_ratio * energy_per_write;
}
...
channel_conf *clone(channel_conf *origin) {
  ...
  channel_conf *new_channel =
      new channel_conf(origin->memcad_params, temp, origin->bandwidth,
                       origin->type, origin->low_power);
  return new_channel;
}
...
ostream &operator<<(ostream &os, const channel_conf &ch_cnf) {
  ...
  os << " DIMM: "
     << ((ch_cnf.type == UDIMM)
             ? " UDIMM "
             : ((ch_cnf.type == RDIMM) ? " RDIMM " : "LRDIMM "));
...
bob_conf::bob_conf(MemCadParameters *memcad_params,
                   vector<channel_conf *> *in_channels)
    : memcad_params(memcad_params), num_channels(0), capacity(0), bandwidth(0),
      energy_per_read(0), energy_per_write(0), energy_per_access(0), cost(0),
      latency(0), valid(true) {
...
memory_conf::memory_conf(MemCadParameters *memcad_params,
                         vector<bob_conf *> *in_bobs)
