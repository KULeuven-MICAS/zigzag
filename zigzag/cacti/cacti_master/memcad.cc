...
void find_all_channels(MemCadParameters *memcad_params) {
  ...
  for (int d1 = 0; d1 < 6; d1++) {
    for (int d2 = d1; d2 < 6; d2++) {
      for (int d3 = d2; d3 < 6; d3++) {
        ...
        if ((DIMM_size[d1] + DIMM_size[d2] + DIMM_size[d3]) >
            memcad_params->capacity)
          continue;

        if (((current_dimm_model == JUST_LRDIMM) ||
             (current_dimm_model == ALL)) &&
            ((d1 == 0) ||
             (MemoryParameters::cost[current_io_type][2][d1 - 1] < INF)) &&
            ((d2 == 0) ||
             (MemoryParameters::cost[current_io_type][2][d2 - 1] < INF)) &&
            ((d3 == 0) ||
             (MemoryParameters::cost[current_io_type][2][d3 - 1] < INF))) {
          int num_dimm_per_channel = 0;
          vector<int> dimm_cap;
          dimm_cap.push_back(DIMM_size[d1]);
          if (d1 > 0)
            num_dimm_per_channel++;
          dimm_cap.push_back(DIMM_size[d2]);
          if (d2 > 0)
            num_dimm_per_channel++;
          dimm_cap.push_back(DIMM_size[d3]);
          if (d3 > 0)
            num_dimm_per_channel++;

          int max_index = bw_index(
              current_io_type,
              MemoryParameters::bandwidth_load[current_io_type]
                                              [4 - num_dimm_per_channel]);
          for (int bw_id = 0; bw_id <= max_index; ++bw_id) {
            int bandwidth =
                MemoryParameters::bandwidth_load[current_io_type][bw_id];
            channel_conf *new_channel = new channel_conf(
                memcad_params, dimm_cap, bandwidth, LRDIMM, false);
            ...
            if (memcad_params->low_power_permitted) {
              new_channel = new channel_conf(memcad_params, dimm_cap, bandwidth,
                                             LRDIMM, true);
              if (new_channel->cost < INF) {
                memcad_all_channels->push_back(new_channel);
              }
        ...
        if ((current_dimm_model == JUST_RDIMM) ||
            (current_dimm_model == ALL) &&
                ((d1 == 0) ||
                 (MemoryParameters::cost[current_io_type][1][d1 - 1] < INF)) &&
                ((d2 == 0) ||
                 (MemoryParameters::cost[current_io_type][1][d2 - 1] < INF)) &&
                ((d3 == 0) ||
                 (MemoryParameters::cost[current_io_type][1][d3 - 1] < INF))) {
          ...
          for (int bw_id = 0; bw_id <= max_index; ++bw_id) {
            int bandwidth =
                MemoryParameters::bandwidth_load[current_io_type][bw_id];
            channel_conf *new_channel = new channel_conf(
                memcad_params, dimm_cap, bandwidth, RDIMM, false);
            if (new_channel->cost < INF) {
              memcad_all_channels->push_back(new_channel);
            }
        ...
        if ((current_dimm_model == JUST_UDIMM) ||
            (current_dimm_model == ALL) &&
                ((d1 == 0) ||
                 (MemoryParameters::cost[current_io_type][0][d1 - 1] < INF)) &&
                ((d2 == 0) ||
                 (MemoryParameters::cost[current_io_type][0][d2 - 1] < INF)) &&
                ((d3 == 0) ||
                 (MemoryParameters::cost[current_io_type][0][d3 - 1] < INF))) {
          int num_dimm_per_channel = 0;
          vector<int> dimm_cap;
          dimm_cap.push_back(DIMM_size[d1]);
          if (d1 > 0)
          ...
          for (int bw_id = 0; bw_id <= max_index; ++bw_id) {
            int bandwidth =
                MemoryParameters::bandwidth_load[current_io_type][bw_id];
            channel_conf *new_channel = new channel_conf(
                memcad_params, dimm_cap, bandwidth, UDIMM, false);
            if (new_channel->cost < INF) {
              memcad_all_channels->push_back(new_channel);
            }
  ...
  sort(memcad_all_channels->begin(), memcad_all_channels->end(),
       compare_channels);
...
void find_bobs_recursive(MemCadParameters *memcad_params, int start, int end,
                         int nb, list<int> *channel_index) {
  if (nb == 1) {
    for (int i = start; i <= end; ++i) {
      channel_index->push_back(i);

      vector<channel_conf *> temp;
      for (list<int>::iterator it = channel_index->begin();
           it != channel_index->end(); it++) {
        int idx = *it;
...
void find_all_bobs(MemCadParameters *memcad_params) {
  ...
  if (memcad_params->mirror_in_bob) {
    ...
    for (unsigned int i = 0; i < start_index.size(); ++i) {
      find_bobs_recursive(memcad_params, start_index[i], end_index[i],
                          memcad_params->num_channels_per_bob, &channel_index);
    }
...
void find_mems_recursive(MemCadParameters *memcad_params,
                         int remaining_capacity, int start, int nb,
                         list<int> *bobs_index) {
  ...
  if (nb == 1) {
    for (unsigned int i = start; i < memcad_all_bobs->size(); ++i) {
      ...
      for (list<int>::iterator it = bobs_index->begin();
           it != bobs_index->end(); it++) {
        int index = *it;
  ...
  for (unsigned int i = start; i < memcad_all_bobs->size(); i++) {
    ...
    int new_remaining_capacity =
        remaining_capacity - (*memcad_all_bobs)[i]->capacity;
    bobs_index->push_back(i);
    find_mems_recursive(memcad_params, new_remaining_capacity, i, nb - 1,
                        bobs_index);
    bobs_index->pop_back();
  }
}
...
bool find_all_memories(MemCadParameters *memcad_params) {
  ...
  list<int> bobs_index;
  find_mems_recursive(memcad_params, memcad_params->capacity, 0,
                      memcad_params->num_bobs, &bobs_index);

  sort(memcad_all_memories->begin(), memcad_all_memories->end(),
       compare_memories);
  ...
  int min_num_results =
      (memcad_all_memories->size() > 3 ? 3 : memcad_all_memories->size());
