 ...
 * area coefficients, timing parameters, as well as external io configuration
 * parameters - termination values, voltage noise coefficients and
 * voltage/timing noise sensitivity parameters.  More details can be found in
 * the CACTI-IO technical report (), especially Chapters 2 and 3. The user can
 * define new dram types here. */

///////////// DDR3 ///////////////////

const double rtt1_wr_lrdimm_ddr3[8][4] = {
    {INF, INF, 120, 120}, {INF, INF, 120, 120}, {INF, 120, 120, 80},
    {120, 120, 120, 60},  {120, 120, 120, 60},  {120, 80, 80, 60},
    {120, 80, 80, 60},    {120, 80, 60, 40}};
...
const double rtt2_wr_lrdimm_ddr3[8][4] = {
    ...
    {120, 120, 120, 80},                     // 3
    {120, 120, 80, 60},                      // 4
    {120, 120, 80, 60},   {120, 80, 60, 40}, // 6
    {120, 80, 60, 40},    {80, 80, 40, 30}   // 8
};
...
const double rtt1_rd_lrdimm_ddr3[8][4] = {
    ...
    {INF, 120, 120, 80},                     // 3
    {120, 120, 120, 60},                     // 4
    {120, 120, 120, 60},  {120, 80, 80, 60}, // 6
    {120, 80, 80, 60},    {120, 80, 60, 40}  // 8
};
...
const double rtt1_wr_lrdimm_ddr4[8][4] = {
    ...
    {120, 80, 80, 60},                    // 3
    {80, 60, 60, 60},                     // 4
    {80, 60, 60, 60},   {60, 60, 60, 40}, // 6
    {60, 60, 60, 40},   {40, 40, 40, 40}  // 8
};
...
const double rtt2_wr_lrdimm_ddr4[8][4] = {
    ...
    {120, 80, 80, 80},                      // 3
    {80, 80, 80, 60},                       // 4
    {80, 80, 80, 60},     {60, 60, 60, 40}, // 6
    {60, 60, 60, 40},     {60, 40, 40, 30}  // 8
};
...
const double rtt1_rd_lrdimm_ddr4[8][4] = {
    ...
    {120, 80, 80, 60},                    // 3
    {120, 60, 60, 60},                    // 4
    {120, 60, 60, 60},  {80, 60, 60, 40}, // 6
    {80, 60, 60, 40},   {60, 40, 40, 30}  // 8
};
...
const double rtt2_rd_lrdimm_ddr4[8][4] = {
    ...
    {60, 60, 40, 40},                       // 3
    {60, 40, 40, 40},                       // 4
    {60, 40, 40, 40},     {40, 40, 40, 30}, // 6
    {40, 40, 40, 30},     {40, 30, 30, 20}  // 8
};

const double rtt1_wr_host_dimm_ddr4[3][4] = {
    {80, 60, 60, 60}, {60, 60, 60, 60}, {40, 40, 40, 40}};

const double rtt2_wr_host_dimm_ddr4[3][4] = {
    {80, 80, 80, 60}, {60, 60, 60, 40}, {60, 40, 40, 30}};

const double rtt1_rd_host_dimm_ddr4[3][4] = {
    {120, 60, 60, 60}, {80, 60, 60, 40}, {60, 40, 40, 30}};

const double rtt2_rd_host_dimm_ddr4[3][4] = {
    {60, 40, 40, 40}, {40, 40, 40, 30}, {40, 30, 30, 20}};

const double rtt1_wr_bob_dimm_ddr4[3][4] = {
    {120, 80, 80, 60}, {80, 60, 60, 60}, {60, 60, 60, 40}};

const double rtt2_wr_bob_dimm_ddr4[3][4] = {
    {120, 80, 80, 80}, {80, 80, 80, 60}, {60, 60, 60, 40}};

const double rtt1_rd_bob_dimm_ddr4[3][4] = {
    {120, 80, 80, 60}, {120, 60, 60, 60}, {80, 60, 60, 40}};

const double rtt2_rd_bob_dimm_ddr4[3][4] = {
    {60, 60, 40, 40}, {60, 40, 40, 40}, {40, 40, 40, 30}};
...
IOTechParam::IOTechParam(InputParameter *g_ip) {
  ...
  num_mem_clk = g_ip->num_mem_dq * (g_ip->num_dq / g_ip->mem_data_width) /
                (g_ip->num_clk / 2);
  ...
  if (g_ip->io_type == LPDDR2) { // LPDDR
    ...
    k_noise_read_sen = k_noise_read * (1 + 0.2 * (r_on / 34 - 1) +
                                       0.2 * (g_ip->num_mem_dq / 2 - 1));
    k_noise_addr_sen =
        k_noise_addr * (1 + 0.1 * (rtt_ca / 100 - 1) + 0.2 * (r_on / 34 - 1) +
                        0.2 * (num_mem_ca / 16 - 1));
    ...
    t_jitter_hold_sen = t_jitter_hold * (1 + 0.1 * (r_on / 34 - 1) +
                                         0.3 * (g_ip->num_mem_dq / 2 - 1));
    t_jitter_addr_setup_sen =
        t_jitter_addr_setup *
        (1 + 0.2 * (rtt_ca / 100 - 1) + 0.1 * (r_on / 34 - 1) +
         0.4 * (num_mem_ca / 16 - 1));
    t_jitter_addr_hold_sen =
        t_jitter_addr_hold *
        (1 + 0.2 * (rtt_ca / 100 - 1) + 0.1 * (r_on / 34 - 1) +
         0.4 * (num_mem_ca / 16 - 1));
    ...
    t_jitter_addr_setup_sen =
        t_jitter_addr_setup *
        (1 + 0.1 * (r_on / 50 - 1) + 0.4 * (num_mem_ca / 16 - 1));
  ...
  } else if (g_ip->io_type == DDR3) { // Default parameters for DDR3
    // IO Supply voltage (V)
    vdd_io = 1.5;
    v_sw_clk = 0.75;
    ...
    k_noise_write_sen =
        k_noise_write *
        (1 + 0.1 * (rtt1_dq_write / 60 - 1) + 0.2 * (rtt2_dq_write / 60 - 1) +
         0.2 * (r_on / 34 - 1) + 0.2 * (g_ip->num_mem_dq / 2 - 1));

    k_noise_read_sen =
        k_noise_read *
        (1 + 0.1 * (rtt1_dq_read / 60 - 1) + 0.2 * (rtt2_dq_read / 60 - 1) +
         0.2 * (r_on / 34 - 1) + 0.2 * (g_ip->num_mem_dq / 2 - 1));

    k_noise_addr_sen =
        k_noise_addr * (1 + 0.1 * (rtt_ca / 50 - 1) + 0.2 * (r_on / 34 - 1) +
                        0.2 * (num_mem_ca / 16 - 1));

    t_jitter_setup_sen =
        t_jitter_setup *
        (1 + 0.2 * (rtt1_dq_write / 60 - 1) + 0.3 * (rtt2_dq_write / 60 - 1) +
    ...
    t_jitter_addr_setup_sen =
        t_jitter_addr_setup *
        (1 + 0.2 * (rtt_ca / 50 - 1) + 0.1 * (r_on / 34 - 1) +
         0.4 * (num_mem_ca / 16 - 1));
  ...
  } else if (g_ip->io_type == DDR4) { // Default parameters for DDR4
    // IO Supply voltage (V)
    vdd_io = 1.2;
    v_sw_clk = 0.6;
  ...
  } else if (g_ip->io_type == Serial) { // Default parameters for Serial
    // IO Supply voltage (V)
    vdd_io = 1.2;
    v_sw_clk = 0.75;
  ...
  // Swing calculation
  v_sw_data_read_load1 =
      vdd_io * (rtt1_dq_read) * (rtt2_dq_read + rs2_dq) /
      ((rtt1_dq_read + rtt2_dq_read + rs2_dq) * (r_on + rs1_dq + rpar_read));
  v_sw_data_read_load2 =
      vdd_io * (rtt1_dq_read) * (rtt2_dq_read) /
      ((rtt1_dq_read + rtt2_dq_read + rs2_dq) * (r_on + rs1_dq + rpar_read));
  v_sw_data_read_line = vdd_io * rpar_read / (r_on + rs1_dq + rpar_read);
  v_sw_addr = vdd_io * rtt_ca / (50 + rtt_ca);
  v_sw_data_write_load1 =
      vdd_io * (rtt1_dq_write) * (rtt2_dq_write + rs2_dq) /
      ((rtt1_dq_write + rs1_dq + rtt2_dq_write + rs2_dq) * (r_on + rpar_write));
  v_sw_data_write_load2 =
      vdd_io * (rtt2_dq_write) * (rtt1_dq_write + rs1_dq) /
      ((rtt1_dq_write + rs1_dq + rtt2_dq_write + rs2_dq) * (r_on + rpar_write));
...
IOTechParam::IOTechParam(InputParameter *g_ip, Mem_IO_type io_type1,
                         int num_mem_dq, int mem_data_width, int num_dq,
                         int connection, int num_loads, double freq) {
  ...
  if (io_type == LPDDR2) { // LPDDR
    ...
    k_noise_write_sen = k_noise_write * (1 + 0.2 * (r_on / 34 - 1) +
                                         0.2 * (num_mem_dq / 2 - 1));
    k_noise_read_sen =
        k_noise_read * (1 + 0.2 * (r_on / 34 - 1) + 0.2 * (num_mem_dq / 2 - 1));
    k_noise_addr_sen =
        k_noise_addr * (1 + 0.1 * (rtt_ca / 100 - 1) + 0.2 * (r_on / 34 - 1) +
                        0.2 * (num_mem_ca / 16 - 1));
    ...
    k_noise_write_sen = k_noise_write * (1 + 0.2 * (r_on / 50 - 1) +
                                         0.2 * (num_mem_dq / 2 - 1));
    k_noise_read_sen =
        k_noise_read * (1 + 0.2 * (r_on / 50 - 1) + 0.2 * (num_mem_dq / 2 - 1));
    k_noise_addr_sen = k_noise_addr * (1 + 0.2 * (r_on / 50 - 1) +
                                       0.2 * (num_mem_ca / 16 - 1));
  ...
  } else if (io_type == DDR3) { // Default parameters for DDR3
    // IO Supply voltage (V)
    vdd_io = 1.5;
    v_sw_clk = 0.75;
    ...
    */
    ...
    case (0):
      rtt1_dq_write =
          rtt1_wr_bob_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_write =
          rtt2_wr_bob_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt1_dq_read =
          rtt1_rd_bob_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_read =
          rtt2_rd_bob_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      break;
    case (1):
      rtt1_dq_write =
          rtt1_wr_host_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_write =
          rtt2_wr_host_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt1_dq_read =
          rtt1_rd_host_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_read =
          rtt2_rd_host_dimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      break;
    case (2):
      rtt1_dq_write =
          rtt1_wr_lrdimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_write =
          rtt2_wr_lrdimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt1_dq_read =
          rtt1_rd_lrdimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_read =
          rtt2_rd_lrdimm_ddr3[num_loads - 1][frequnecy_index(io_type)];
      break;
    default:
      break;
    }
    ...
    k_noise_write_sen =
        k_noise_write *
        (1 + 0.1 * (rtt1_dq_write / 60 - 1) + 0.2 * (rtt2_dq_write / 60 - 1) +
         0.2 * (r_on / 34 - 1) + 0.2 * (num_mem_dq / 2 - 1));
  ...
  } else if (io_type == DDR4) { // Default parameters for DDR4
    // IO Supply voltage (V)
    vdd_io = 1.2;
    v_sw_clk = 0.6;
    ...
    */
    ...
    case (0):
      rtt1_dq_write =
          rtt1_wr_bob_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_write =
          rtt2_wr_bob_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt1_dq_read =
          rtt1_rd_bob_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_read =
          rtt2_rd_bob_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      break;
    case (1):
      rtt1_dq_write =
          rtt1_wr_host_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_write =
          rtt2_wr_host_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt1_dq_read =
          rtt1_rd_host_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_read =
          rtt2_rd_host_dimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      break;
    case (2):
      rtt1_dq_write =
          rtt1_wr_lrdimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_write =
          rtt2_wr_lrdimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt1_dq_read =
          rtt1_rd_lrdimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      rtt2_dq_read =
          rtt2_rd_lrdimm_ddr4[num_loads - 1][frequnecy_index(io_type)];
      break;
    default:
      break;
    }
  ...
  } else if (io_type == Serial) { // Default parameters for Serial
    // IO Supply voltage (V)
    vdd_io = 1.2;
    v_sw_clk = 0.75;
