...
class MinimalEnergyStage(Stage):
    ...
    def __init__(self, list_of_callables, *, reduce_minimal_keep_others=False, **kwargs):
        ...
        self.best_cme: CostModelEvaluation = None  # type: ignore
        # Visualization stuff
        self.energies = []
    ...
    def run(self) -> Generator[tuple[CostModelEvaluation, Any], None, None]:
        ...
        for cme, extra_info in substage.run():
            ...
            if (
                ...
                or (
                    cme.energy_total == self.best_cme.energy_total and cme.latency_total2 < self.best_cme.latency_total2
                )
            ):
                self.best_cme = cme
            if self.keep_others:
    ...
    def run(self) -> Generator[Tuple[CostModelEvaluation, Any], None, None]:
        ...
        for cme, extra_info in substage.run():
            ...
            if (
                ...
                or (
                    cme.latency_total2 == self.best_cme.latency_total2 and cme.energy_total < self.best_cme.energy_total
                )
            ):
                self.best_cme = cme
            if self.keep_others:
...
class MinimalEDPStage(Stage):
    ...
    def __init__(self, list_of_callables, *, reduce_minimal_keep_others=False, **kwargs) -> None:
        """!  The class constructor
        Initialize the compare stage.
        """
        super().__init__(list_of_callables, **kwargs)
    ...
    def run(self) -> Generator[Tuple[CostModelEvaluation, Any], None, None]:
        ...
        for cme, extra_info in substage.run():
            ...
            if (
                self.best_cme is None
                or cme.latency_total2 * cme.energy_total < self.best_cme.latency_total2 * self.best_cme.energy_total
            ):
                self.best_cme = cme
            if self.keep_others:
                other_cmes.append((cme, extra_info))
...
class SumStage(Stage):
    ...
    def __init__(self, list_of_callables, **kwargs):
        """!  Initialize the compare stage."""
        super().__init__(list_of_callables, **kwargs)
        self.total_cme: CostModelEvaluation = None  # type: ignore
...
class ListifyStage(Stage):
    ...
    def __init__(self, list_of_callables, **kwargs) -> None:
        """!  Initialize the compare stage."""
        super().__init__(list_of_callables, **kwargs)
