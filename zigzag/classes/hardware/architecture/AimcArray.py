...
class AimcArray(ImcUnit):
    ...
    def get_adc_cost(self):
        ...
        if self.hd_param["adc_resolution"] == 1:
            ...
            k2 = 1.206
            adc_area = (
                10 ** (k1 * self.hd_param["adc_resolution"] + k2)
                * 2 ** self.hd_param["adc_resolution"]
                * (10**-6)
            )  # unit: mm^2
        """delay (ns)"""
        k3 = 0.00653  # ns
        k4 = 0.640  # ns
        adc_delay = self.hd_param["adc_resolution"] * (
            k3 * self.dimensions["D2"] + k4
        )  # unit: ns
        """energy (fJ)"""
        k5 = 100  # fF
        k6 = 0.001  # fF
        adc_energy = (
            k5 * self.hd_param["adc_resolution"]
            + k6 * 4 ** self.hd_param["adc_resolution"]
    ...
    def get_dac_cost(self):
        ...
        if self.hd_param["input_bit_per_cycle"] == 1:
            ...
            dac_energy = (
                ...
                * self.logic_unit.tech_param["vdd"] ** 2
            )  # unit: pJ
    ...
    def get_area(self):
        ...
        # area of ADCs
        area_adcs = (
            self.get_adc_cost()[0] * w_pres * self.wl_dim_size * self.nb_of_banks
        )
        ...
        if nb_inputs_of_adder_pv == 1:
            ...
            nb_of_1b_adder_pv = adder_input_precision * (
        ...
        if self.hd_param["input_bit_per_cycle"] == self.hd_param["input_precision"]:
            ...
            nb_of_1b_reg_accumulator = (
                nb_of_1b_adder_accumulator  # number of regs in an accumulator
            )
            area_accumulators = (
    ...
    def get_peak_energy_single_cycle(self):
        ...
        energy_cells = (
            (
                self.logic_unit.tech_param["bl_cap"]
                * (self.logic_unit.tech_param["vdd"] ** 2)
...
if __name__ == "__main__":
    ...
    hd_param_aimc["adc_resolution"] = hd_param_aimc[
        "input_bit_per_cycle"
    ] + 0.5 * math.log2(dimensions["D2"])
