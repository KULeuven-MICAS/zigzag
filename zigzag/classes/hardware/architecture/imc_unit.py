...
class LogicUnit:
    ...
    def __init__(self, tech_param: dict):
        ...
        self.tech_param["bl_cap"] = (
            tech_param["nd2_cap"] / 2
        )  # bitline cap of each SRAM cell is treated as NAND2_cap/2
    ...
    def check_tech_param(self, tech_param):
        ...
        for ii_a, a in enumerate(required_param):
            if a not in tech_param.keys():
                raise Exception(
                    f"[LogicUnit] Incorrect input, required param [{a}] not found."
                )
            if not (isinstance(tech_param[a], int) or isinstance(tech_param[a], float)):
                raise Exception(
                    f"[LogicUnit] Incorrect input, value [{tech_param[a]}] of param [{a}] is not a num."
                )
            if tech_param[a] <= 0:
                raise Exception(
                    f"[LogicUnit] Incorrect input, value [{tech_param[a]}] of param [{a}] is not positive."
                )
    ...
    def get_1b_multiplier_energy(self):
        ...
        return (
            0.5 * self.tech_param["nd2_cap"] * (self.tech_param["vdd"] ** 2)
        )  # unit: pJ
...
class ImcUnit:
    ...
    def __init__(self, tech_param: dict, hd_param: dict, dimensions: dict):
        ...
        self.bl_dim_size = dimensions[
            self.bl_dim
        ]  # dimension where bitline (adder tree) is
        self.nb_of_banks = math.prod(
            [
                dimensions[oa_dim]
                for oa_dim in dimensions
                if oa_dim not in [self.wl_dim, self.bl_dim]
            ]
        )
    ...
    def check_input(self, hd_param, dimensions):
        ...
        for ii_a, a in enumerate(required_hd_param):
            if a not in hd_param.keys():
                raise Exception(
                    f"[ImcArray] Incorrect hd_param, required param [{a}] not found."
                )
            if a == "imc_type":
                ...
                if (
                    not isinstance(hd_param[a], str)
                    or hd_param[a] not in dimensions.keys()
                ):
                    raise Exception(
                        f"[ImcArray] param [{a}] is not a str or is not a key in dimensions."
                    )
            elif a == "enable_cacti":
                if not isinstance(hd_param[a], bool):
                    raise Exception(
                        f"[ImcArray] param [{a}] is not bool (Ture, False)."
                    )
            else:
                ...
                if (
                    a == "input_bit_per_cycle"
                    and hd_param[a] > hd_param["input_precision"]
                ):
        ...
        if hd_param["imc_type"] == "analog":
            ...
            if (
                not (isinstance(hd_param[a], int) or isinstance(hd_param[a], float))
            ) or (hd_param[a] <= 0):
                raise Exception(
    ...
    def get_single_cell_array_cost_from_cacti(
        self, tech_node, wl_dim_size, bl_dim_size, group_depth, w_pres
    ):
